"""The pre-v1.2 regex translator, kept only as a test oracle.

This is how W++ used to be translated: one regular-expression pass over the
source, swapping keywords while stepping around strings and comments.  It was
replaced by the AST compiler in wpplang/compiler, and it is NOT on any
execution path - nothing in wpplang imports it.

It lives here because it is a useful second opinion.  Both it and the compiler
target Python, so for any valid W++ program the two must produce Python that
parses to the same AST.  tests/test_compiler_equivalence.py checks exactly
that, which is how the refactor was shown to change no behaviour.

The original notes follow.

Two rules keep the swap honest:
...
from wpplang.keywords import KEYWORDS
